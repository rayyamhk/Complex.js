...
class Complex {
  ...
  getArgument() {
    const x = this.re;
    const y = this.im;
    if (Math.abs(x) < Complex.EPSILON && Math.abs(y) < Complex.EPSILON) {
      return undefined;
    }
    if (x === 0) {
      if (y > 0) {
...
Number.isNaN = Number.isNaN || function _isNaN(value) {
  return typeof input === 'number' && isNaN(value);
};
...
Number.isInteger = Number.isInteger || function _isInteger(value) {
